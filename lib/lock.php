...
class Lock
{
	 ...
	 *
	 * ⚠ Рубильник `off` проверяется и здесь, и раньше — в `currentStage()`. Дубль
	 * намеренный: `currentStage()` обязан погасить рубильником ещё и обход из
	 * адреса, а эта функция публичная и должна оставаться верной сама по себе,
	 * без оглядки на вызывающего. Меняешь смысл `off` — правь оба места.
	 *
	 ...
	 */
	/**
	 * ⚠ Обход может быть погашен ДО этого вызова: рубильник `off` и «приостановка
	 * не про этого пользователя» выходят из `currentStage()` раньше. Если
	 * `?lock=hard` не показывает экран — смотри туда, а не сюда.
	 */
