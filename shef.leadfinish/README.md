...
| `'value' => ['abc']` — мусор | никому: опечатка не должна раскатывать кнопку |
...
'lock' => [
    'value' => [
        ...
        'users' => [562],              // кого касается; пусто — всех
    ],
    'readonly' => false,
],
```
...
дата отбрасывается, мусор в `off` выключателем не считается. Запереть того, кто
