...
Установка регистрирует обработчик `main::OnEpilog` и создаёт настройку
`allowed_users` со значением `44,562`.

## Удаление

Там же кнопкой **Удалить**. Снимается обработчик события и настройки модуля;
файлы каталога остаются — их удалять вручную.
...
**Marketplace → Установленные решения → модуль → «Настроить»**, поле
«ID пользователей»: список через запятую, например `44,562`.

**Пустое поле = кастомизация работает для всех пользователей.** Так задумано:
после обкатки на нескольких людях включить её на всех можно просто очистив поле,
без правки кода.
...
```php
\Bitrix\Main\Config\Option::set('shef.leadfinish', 'allowed_users', '44,562');
\Bitrix\Main\Config\Option::set('shef.leadfinish', 'allowed_users', '');  // всем
```
...
| `.settings.php` | контроллеры и расписание приостановки |
...
- в консоли нет строки «слушатель попапа установлен» → скрипт не подключился:
  проверить `allowed_users` и что адрес страницы — детальная лида;
