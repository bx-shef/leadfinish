...
| Место | Кнопка подменяется |
| --- | --- |
| Карточка лида, в том числе открытая в слайдере | да |
| Список лидов | да |
| Канбан лидов | **нет** — там окно выбора устроено иначе |

Канбан не поддержан намеренно: ядро рисует там другое окно, без тех опорных
точек, за которые цепляется доработка. Штатное поведение канбана при этом
сохраняется полностью — менеджер завершает лид как раньше.
