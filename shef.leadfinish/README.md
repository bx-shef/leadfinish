...
Порядок здесь важен: шаг 3 правит файл, который распаковка на шаге 2
перетирает. Сделать наоборот — список вернётся к поставочному, и кнопку никто
не увидит.
...
Именно `local/`, а не `bitrix/`: каталог `bitrix/modules/` — территория
поставки, её перетирает обновление платформы.
...
### 4. Установить через админку

Админка → **Marketplace → Установленные решения** (или
`/bitrix/admin/partner_modules.php`) → «[SH-local] Своя кнопка завершения лида»
→ **Установить**.
...
Порядок тот же, но с одной оговоркой: **`unzip -o` перетирает `.settings.php`**
вместе со списком доступа и расписанием приостановки.

```bash
# 1. Посмотреть и выписать, что стоит сейчас
cat /home/bitrix/www/local/modules/shef.leadfinish/.settings.php
...
# 3. Вернуть свои значения в .settings.php
```

Чтобы третий шаг пропал сам собой, те же правки нужно держать **и в
репозитории**: тогда следующая поставка приезжает уже с нужным списком. Файл на
сервере — копия, а не источник правды.

Переустановка через админку нужна, только если менялись установочные события.
Менялись `lib/`, `js/`, `css/` — хватит распаковки и сброса кеша.
...
JS и CSS подключаются **только** на страницах лида — карточка, список,
канбан — и только пользователям из списка: скрипт вешает глобальный слушатель
попапов, и на остальных страницах портала он не нужен.
