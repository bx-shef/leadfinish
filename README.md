...
⚠ `composer.json` должен лежать **в корне портала**: путь к модулю считается
относительно него. Из подкаталога модуль уедет в `local/bitrix/modules/...` —
молча. Как это обойти и что ещё проверить — в
[`docs/install-via-composer.md`](docs/install-via-composer.md).
...
⚠ **Ставить из консоли не надо.** `ModuleManager::registerModule()` только
помечает модуль установленным: он не регистрирует обработчик события и не
копирует фронт. Получится модуль, который числится установленным и ничего не
делает, а искать причину будете в коде.
