...
Поставка на постоянной ссылке — тегом от `main`:
...
Дальше всё делает CI: сверяет тег с `VERSION` внутри модуля (не совпали —
релиза не будет), собирает архив и выкладывает релиз с приложенным
`shef.leadfinish.zip` и списком изменений с прошлого тега.
