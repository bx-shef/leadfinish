...
1. Скачать `shef.leadfinish.zip` со [страницы релизов](https://github.com/bx-shef/leadfinish/releases)
   и распаковать в портал:
...
В `composer.json` проекта нужно **один раз** указать, куда класть модуль:
...
по умолчанию ставится в `bitrix/modules/`, то есть в каталог ядра, куда локальным
модулям нельзя. Строка выше перенаправляет его в `local/modules/`.

Composer только раскладывает файлы. **Установить модуль** всё равно нужно через
**Marketplace → Установленные решения** или из консоли:

```php
\Bitrix\Main\ModuleManager::registerModule('shef.leadfinish');
```

Обновление — `composer update shef/leadfinish`; переустанавливать модуль в
админке при этом не требуется, если не менялись события установки.

⚠ Обновление **перетирает `.settings.php`** вместе со списком `allowed_users` и
расписанием приостановки. Настройки, правленные на сервере, держите в своём
репозитории — иначе следующая поставка вернёт поставочные.

### Что делает установка

Регистрируется обработчик `main::OnEpilog`. Настроек в базе модуль не создаёт:
список тех, кому доступна кастомизация, лежит в `.settings.php` и приезжает
вместе с поставкой.
